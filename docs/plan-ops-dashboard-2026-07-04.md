...
| D | 访问方式 | **D2 公网 + 密码** |
| E | 分期 | **E1 一期闭环**,二期(巡检+push 页)在真数据上迭代 |
| UI | 布局定稿 | **方案 B · 深色控制台 + 左侧导航栏**(2026-07-04 三方案比选拍板 · 布局基准 = `docs/mockup-ops-b.html`,执行按它实现)。**一期五页**:总览 / 告警 / 源管理 / **博文管理**(老板点名提前一期:按 博客/管线/标题/正文/发布时间/采集时间/push状态 筛选)/ **错误日志**(老板点名:每条错误可见 代理错/不可达/429/500/400/404 等具体类型) |
...
CREATE TABLE IF NOT EXISTS articles (  -- 🆕 博文查询真源(dashboard/分析器读这里 · 不扫 dataset 文件)
  ...
  crawled_at TEXT,
  push_status TEXT DEFAULT 'none',   -- none(未推)/ pushed / failed · push 对接后由 push 流程回写
  pushed_at TEXT,
  PRIMARY KEY (url, token_id)        -- 1-to-N 共享博客兼容(push 记忆:同 url 多 token 合法)
);

CREATE TABLE IF NOT EXISTS crawl_errors ( -- 🆕 错误明细(老板点名:看清每条错误是代理/不可达/429/500/404 什么原因)
  err_id INTEGER PRIMARY KEY AUTOINCREMENT,
  run_id TEXT NOT NULL,
  token_id INTEGER, base_symbol TEXT,
  url TEXT,
  kind TEXT NOT NULL,                -- proxy_error / unreachable / timeout / http_403 / http_404 / http_429 / http_4xx / http_5xx / tls_error / parse_error / internal
  http_status INTEGER,
  message TEXT,                      -- 原始错误信息(截 300 字)
  retries INTEGER,
  at TEXT
);
-- 留存:crawl_errors 按 run 保留 30 天(run-batch 收尾清理过期行 · 防库膨胀)
...
**错误明细采集:** crawlee `failedRequestHandler`/`errorHandler`(每 crawler 挂一次)拿 (request, error) → `shared/error-classify.ts` 纯函数分类(error.message/statusCode → kind 枚举:impit 代理错→proxy_error · ENOTFOUND/ECONNREFUSED→unreachable · 超时→timeout · Request blocked 403→http_403…)→ 内存 buffer 轮末入 crawl_errors。rss/substack 直拉 catch 分支同走分类器。分类器单测覆盖全部枚举。
...
**布局基准 = `docs/mockup-ops-b.html`(老板拍板)**:深色控制台(暖黑 #1B1A17 · 等宽字体 · 状态色发光点)· 左侧固定导航(五项 + 栏底常驻调度状态)。

| 页 | 内容 |
|---|---|
| **总览** | ①脉搏行(上次批次:状态/**完成时间**/耗时/新增/失败/吞吐 · 下次倒计时)②24 批次健康带(绿黄红格 · hover 详情)③三卡(今日新增/活跃告警/有数据源)④最近批次表 |
| **告警** | open 告警表(级别/类型/源/持续轮数/detail/ack);已 ack/已恢复折叠;跳源详情 |
| **源管理** | 全源表(模糊搜/管线/处置/告警筛;列:symbol/URL/管线/**最后出文**/7天新增/近轮失败/告警灯)· 点行展开单源详情(30 轮柱状+最近博文+告警史)。不采集源按处置筛可见 |
| **博文管理**(一期 · 老板点名)| articles 全库:筛 **标题/正文关键词 · 博客 · 管线 · push 状态(未推/已推/失败)· 发布时间范围 · 采集时间范围**;列:标题(点开:正文预览/字段质量/原文链)/博客/管线/发布/采集/push。API `GET /api/articles?q=&symbol=&crawler=&push=&pub_from=&pub_to=&crawled_from=&crawled_to=&page=` |
| **错误日志**(一期 · 老板点名)| ①错误分布卡(本轮/各 kind 计数)②筛:批次·类型枚举·源/URL 搜 ③明细表:时间/源/URL/**类型 chip**/HTTP 码/原始信息/重试。API `GET /api/errors?run=&kind=&q=&page=`。同源同类连续错由 detector 聚合成告警(两页联动) |

二期:push 账本页 · 巡检 diff 页(博文检索已提前一期)。
...
| 6 | dashboard 五页 | 公网密码可访问;五页数据与 sqlite 直查一致;ack 生效;博文页各筛选维度实测;错误页 kind 分类与真实错误抽查 10 条一致;**playwright 截图五页附验收报告**(铁律:UI 必真访问自验)|
| 6b | 错误分类器 | error-classify 单测覆盖全部 kind(每类真实 message fixture);crawl_errors 30 天清理实测 |
| 7 | 文档 | ops/README.md(装/卸/改频率/改阈值/reset SOP)+ handoff 补记 |

### 二期(账本跑出真数据后另出计划)

巡检批次(结构 diff 告警)· push_runs 激活 + articles.push_status 回写(等 push 对接 · 注意 push 记忆:同 url 多 token 合并一条 + 首次接通存量不推)· seen-store 裁剪 · C3 通道落地 · HTTPS 完善。
